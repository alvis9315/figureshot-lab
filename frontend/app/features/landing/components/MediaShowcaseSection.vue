<template>
  ...
  <div class="mx-auto max-w-6xl px-4 py-10 md:py-12">
    ...
    <div class="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4">
      <!-- 小卡:白版 #02a8e0 白字、深版黃底深字(2026-07-12) -->
      <div v-for="thumb in thumbs" :key="thumb.scene" class="group relative overflow-hidden rounded-xl">
        <BaseMedia
          ...
          :fallback="isLight ? 'background:#02a8e0' : 'background: var(--fs-accent)'"
        />
        <div class="absolute inset-x-0 bottom-0 p-3">
          <p class="text-sm font-medium" :class="isLight ? 'text-white' : 'text-[#17161a]'">{{ $t(`style.scene.${thumb.scene}`) }}</p>
          <p class="font-mono text-[10px] uppercase tracking-wider" :class="isLight ? 'text-[#dfe3e6]' : 'text-[#5a4522]'">{{ $t(`style.mood.${thumb.mood}`) }}</p>
        </div>
      </div>
    </div>
  </div>
