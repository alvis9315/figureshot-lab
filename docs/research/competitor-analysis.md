...
> 初稿 2026-07-12(WebSearch 調研)。狀態:暫定,待擁有者審。
...
| 產品 | 定位 | 核心功能 | 沒做的 |
|---|---|---|---|
| **MyFigureCollection.net(MFC)** | 模型圈最大資料庫社群 | 公共模型資料庫、Owned/Wished/Ordered 標記、價格比較、發售月曆、收藏統計圖表、圖片分享 | 拍攝靈感、搭配建議、AI 文案——收藏是「檔案櫃」,不是「創作起點」 |
...
1. **不要跟 MFC 拚資料庫**——第一版堅持「使用者自建收藏」(spec 第 19 節已定),把力氣放在 MFC 沒有的生成體驗;未來再談匯入。
2. **輪盤/產生器是護城河**,文案只是出口——行銷敘事順序應為「收藏活起來」>「AI 幫你寫文案」。
3. 訪談(PRE-002)加一題:「你用過 MFC 嗎?為什麼(不)繼續用?」——直接聽既有方案的流失原因。
